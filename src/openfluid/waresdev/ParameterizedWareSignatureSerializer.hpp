...
/**
  @file ParameterizedWareSignatureSerializer.hpp
...
inline std::string getCPPValueType(const openfluid::core::Value::Type ValueType) //TOIMPL group converter functions?
{
  switch (ValueType)
  {
...
class OPENFLUID_API ParameterizedWareSignatureSerializer : public WareSignatureSerializer<SignatureType>
{
  protected:
    ...
    std::string getCPPDataString(const std::string Member, 
                                 const std::vector<openfluid::ware::SignatureDataItem>& Data) const;
...
template<class SignatureType>
void ParameterizedWareSignatureSerializer<SignatureType>::unserializeParametersFromJSON(
                                                            const openfluid::thirdparty::json& Json, 
                                                            SignatureType& Sign) const
{
                                                    ...
                                                    const std::vector<openfluid::ware::SignatureDataItem>& Data) const
{
  ...
  for (const auto& D : Data)
  {
    ...
    Str += WareSignatureSerializer<SignatureType>::getCPPMethod(
      Member,"push_back",{"{"+ParameterizedWareSignatureSerializer<SignatureType>::getQuotedString(D.Name)+","+
                         ParameterizedWareSignatureSerializer<SignatureType>::getQuotedString(D.Description)+","+
                         ParameterizedWareSignatureSerializer<SignatureType>::getQuotedString(D.SIUnit)+","+
...
std::string ParameterizedWareSignatureSerializer<SignatureType>::toWareCPPParams(const SignatureType& Sign) const
{
  ...
  // Parameters
  CPP += ParameterizedWareSignatureSerializer<SignatureType>::getCPPDataString("HandledData.UsedParams",
                                                                               Sign.HandledData.UsedParams);
  CPP += ParameterizedWareSignatureSerializer<SignatureType>::getCPPDataString("HandledData.RequiredParams",
                                                                               Sign.HandledData.RequiredParams);
