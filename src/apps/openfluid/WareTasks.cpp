...
int WareTasks::processSetup() const
{
  ...
  //curl http://147.100.175.211:8181/api/wares/sets -H "Accept: application/x.openfluid.fluidhub+json; version=1.0"
    ...
    if (FHClient.connect(SourceURL,false))//TOIMPL
    {
      ...
      for (const auto& Wareset : Waresets)
      {
        ...
        {
          WaresetName = ID;
          WaresetListJson = Wareset.second;
          Found = true;
        }
      }
      if (!Found)
  ...
  else if (WaresetSource == "dataset")
  {
    ...
    auto FXDesc = FXIO.loadFromDirectory(m_Cmd.getOptionValue("dataset"));
  ...
  bool BuildTogether = m_Cmd.isOptionActive("parallel-builds");
  if (BuildTogether)
  {
