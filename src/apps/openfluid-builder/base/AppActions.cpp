...
void AppActions::updateExtensionsActionsAndMenus()
{
  ...
  for (it = itb; it!= ite; ++it)
  {
    ...
    if ((*it).second->Signature->Category == openfluid::builderext::ExtensionCategory::CAT_SPATIAL)
    {
      mp_SpatialExtensionsMenu->addAction(m_ExtensionsActions[(*it).first]);
    }
    else if ((*it).second->Signature->Category == openfluid::builderext::ExtensionCategory::CAT_MODEL)
    {
      mp_ModelExtensionsMenu->addAction(m_ExtensionsActions[(*it).first]);
    }
    else if ((*it).second->Signature->Category == openfluid::builderext::ExtensionCategory::CAT_RESULTS)
    {
