...
void ExtensionsRegistry::registerExtensions()
{
  ...
  {
    if (ExtVector[i]->Signature->Type == openfluid::builderext::ExtensionType::TYPE_FEATURE)
    {
      m_FeatureExtensions[ExtVector[i]->Signature->ID] = ExtVector[i];
    }
    else if (ExtVector[i]->Signature->Type == openfluid::builderext::ExtensionType::TYPE_PARAMETERIZATION)
    {
...
void ExtensionsRegistry::releaseAllFeatureExtensions()
{
  ...
  for (it=itb;it!=ite;++it)
  {
    ...
    if ((*it).second->Body != nullptr && 
        ...
        (*it).second->Signature->Mode == openfluid::builderext::ExtensionMode::MODE_MODELESS)
    {
...
openfluid::builderext::ExtensionMode ExtensionsRegistry::getExtensionMode(const openfluid::ware::WareID_t& ID) const
{
  ...
  return openfluid::builderext::ExtensionMode::MODE_UNKNOWN;
}
