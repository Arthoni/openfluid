...
@warning For data safety reasons, we strongly advise against calling the wsetup-wareset comman in your .openfluid or other work userdata folder. Folder move should be done by hand if a transfer into a working folder is wanted.

For a simple case we can call:
...
To do so several files will be added in `wares-dev` folder:
